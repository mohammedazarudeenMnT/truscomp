...
import { Save, Building2, Plus, Trash2, ChevronUp, ChevronDown, Settings as SettingsIcon, Loader2, Mail, Send, Palette } from "lucide-react";
...
// import { ImageUpload } from "@/components/ui/image-upload"; // Removed
    ...
    const fetchSettings = async () => {
        try {
            ...
            if (response.success && response.data) {
                ...
                // SEO and Hero settings fetch logic removed
            ...
            if (response?.success) {
                ...
                if (response.emailChanged) {
                    toast.info("Verification Email Sent", {
                        description: "Please check the new email address to verify the change.",
                        duration: 8000,
                    });
                }
            }
                        ...
                        <div className="space-y-6">
                            ...
                            {pendingEmailChange && (
                                <div className="bg-yellow-50 border border-yellow-200 text-yellow-800 rounded-lg p-4 flex items-start gap-3">
                                    ...
                                    <div>
                                        <h4 className="font-semibold text-sm">Pending Email Verification</h4>
                                        <p className="text-sm mt-1">
                                            A request to change the company email to <strong className="font-medium">{pendingEmailChange}</strong> is pending. 
                                            Please check that email inbox for a verification link to complete the change.
                                        </p>
                                    </div>
                                </div>
                            )}
                                                ...
                                                onClick={async () => {
                                                    ...
                                                    try {
                                                        const response = await testEmailConfiguration({
                                                            testEmail,
                                                            message: "This is a test email from the Truscomp admin panel.",
                                                        });
                    ...
                    {activeTab === "theme" && (
                        <div className="space-y-6">
                            <div>
                                ...
                                <p className="text-sm text-muted-foreground mb-6">
                                    Customize the primary color of your application. This will affect buttons, links, and other interactive elements throughout the platform.
                                </p>
                            ...
                            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
                                ...
                                <p className="text-sm text-blue-800">
                                    After changing the theme color, the page will automatically reload to apply the new color scheme across the entire application.
                                </p>
                            </div>
                        </div>
                    )}
