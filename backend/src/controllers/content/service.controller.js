  ...
  try {
    ...
    if (!heroTitle || !heroDescription) {
      return res.status(400).json({
        success: false,
        message:
          "Please provide required fields: heroTitle and heroDescription",
      });
    }

    // Validate heroTitle is not empty
    if (typeof heroTitle !== "string" || heroTitle.trim().length === 0) {
      return res.status(400).json({
        success: false,
        message: "Hero title is required and cannot be empty",
      });
    }

    // Validate heroDescription is not empty
    if (
      ...
      return res.status(400).json({
        success: false,
        message: "Hero description is required and cannot be empty",
      });
    }

    // Validate slug is provided and unique
    const { slug } = req.body;
